...
function Curries(props) {
  const { curries, veg, nonveg, handleViewDetailsBtnClick } = props
        ...
        {curries.map((item) => (
          <li id='curries-list-item' className='list-item' key={item.id}>
            <div id='recipe-name-cont'>
              ...
              <button id='recipe-view-btn' onClick={() => handleViewDetailsBtnClick(item.id, 'curries')}>
                {item.showDetails ? '-' : '+'}
              </button>
            </div>
            {item.showDetails && (
